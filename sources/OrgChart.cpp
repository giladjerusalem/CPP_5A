...
namespace ariel{
    ...
    OrgChart::Node::Node(string& b):nextLevel(NULL),nextPreOrder(NULL),nextReverse(NULL)
    ,under_me({}),name(b){
    }
    ...
    //add root to constructor - str empty is guarded.
    ...
    OrgChart OrgChart::add_sub(string f, string c)
    {
        ...
        //run employee search function, if not found - cant add new child
    ...
    bool OrgChart::findEmployeeDFS(Node* employee, string& f, string& c){
        ...
        //run through all epmployee's children
        for(Node* n : employee->under_me){
            bool dfs = this->findEmployeeDFS(n,f,c); //run recursive search
            if(dfs){
                return true;
            }
        }
    ...
    OrgChart::iterator::iterator(Node* _cur, int _flag):cur(_cur),flag(_flag)
    {
        //0 - empty iterator
        //1 - level order
        //2 - reverse order
    ...
    void OrgChart::iterator::Reverse()
    {
        ...
        while (!q.empty())
        {
            ...
            for (int i = (int)tmp->under_me.size() - 1; i >= 0; i--)
            {
    ...
    std::ostream & operator<<(std::ostream & stream, OrgChart & chart)
    {
        ...
        while(iter.getCur() != NULL){
            if(iter.getCur()->under_me.empty()){
                ++iter;
                continue;
            }
            cout << "worker name -> " << iter.getCur()->name << ", worker's children: ";
            for(OrgChart::Node* o : iter.getCur()->under_me){
                cout << o->name << ", ";
            }
            cout << "\n";
            ++iter;
